...
function updateForm() {
	if (document.getElementById("sslcontact_form") == null) setTimeout(updateForm);
		...
		replaceFormLabels();
		solveCaptcha();
		document.getElementsByTagName("textarea")[0].addEventListener("input", function() { if (document.getElementsByClassName("newsletter").length == 0) this.style.height = this.scrollHeight + "px"; });
